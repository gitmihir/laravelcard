<script>
    var maxproduct = 10;
        ...
        $(document).on("click", ".add_more_products", function () {
            x++;
            if (x < maxproduct) {
                $(wrapper).append('<div class="parentchild-'+x+'"><div class="duplicatehtml-'+x+'"><table class="table"><tr><td><select product_current_id="' + x + '" name="product_name[]" class="form-control product_name-'+x+'"><option value="">Select Product</option>@foreach ($productselectquery as $productdata)<option product-url="{{ url('/bills/addbills/' . $productdata->product_main_id) }}" product-id="{{ $productdata->product_main_id }}" value="{{ $productdata->product_name }}"> {{ $productdata->product_name }} </option> @endforeach </select></td><td><input type="text" name="product_price[]" class="form-control product_price-' + x + '" readonly></td> <td><input type="number" min="0" current_qty_no='+x+' name="product_quantity[]" class="form-control product_quantity-' + x + '"></td> <td><input type="text" name="product_total_amount[]" class="form-control product_total_amount-' + x + '" readonly></td> <td><button type="button" class="btn btn-danger remove_product"><i class="fas fa-minus-square"></i></button></td></tr><input type="hidden" name="product_id[]" class="product_id-' + x + '"></table></div></div>');
                ...
                $(document).on("change click blur keyup keypress keydown", ".product_quantity-"+x, function () {
                    ...
                    if (price >= 0 && quantity >= 0) {
                        ...
                        $(".parentchild-"+current_qty_no).closest(".parentchild-"+current_qty_no).find(".product_total_amount-"+current_qty_no).val(total_bill_amount.toFixed(2));
                ...
                $(document).on("change", ".product_name-"+x, function () {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            if (price >= 0 && quantity >= 0) {
                                var total_bill_amount = price * quantity;
                                $(".parentchild-"+current_id).closest(".parentchild-"+current_id).find(".product_total_amount-"+current_id).val(total_bill_amount.toFixed(2));
